...
packages.used=c("data.table","lubridate","dplyr","date","reshape2","forecast","doMC","foreach","xgboost","readr","randomForest", "caret", "mlr")
...
plot(n_trees, score_rf, col="blue", xlab="Number of trees in Random Forest", ylab="evaluation score",type="o")
points(n_trees,score_rf,type="o")
rf.par <- n_trees[which.min(score_rf)]
...
if(run.rf){
  ...
  rf_pred <- predict(fit, valid_rf[,-c("unit_sales")])
  ...
  rf_pred1 <- predict(fit, test_rf[,-c("unit_sales")])
