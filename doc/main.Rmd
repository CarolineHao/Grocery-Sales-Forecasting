...
if(run_xgb_stack2){
  ...
  save(preds2, file="../output/XGB2/test_pred_XGB2.RData")
}else{
  load("../output/XGB2/test_pred_XGB2.RData")
}
w <- ifelse(comb_test$perishable == 0, 1, 1.25)
valid_score <- sqrt(sum(w * (log(as.numeric(preds2) + 1) - log(as.numeric(comb_test$unit_sales) + 1))^2)/sum(w))

print(paste0("the score for Xgboost in stack 2 is ", valid_score))
```