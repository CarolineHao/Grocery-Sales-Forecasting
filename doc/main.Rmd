...
if(run.xgboost){
  ...
  features <- c('item_nbr', 'id', 'store_nbr', 'onpromotion', 'year', 'month',
                'dayOfWeek', 'class',
                'perishable', 'dcoilwtico')

  dtrain <- xgb.DMatrix(data = as.matrix(combined_train[, features]), label =
                          combined_train$log_sales)
  dtest <- xgb.DMatrix(data = as.matrix(combined_test[, features]), label =
                         combined_test$log_sales)
  dvalid <- xgb.DMatrix(data = as.matrix(combined_valid[, features]), label =
                          combined_valid$log_sales)
  ...
  rmseErrorsHyperparameters <- apply(searchGridSubCol, 1, function(parameterList){
    ...
    xgboostModelCV <- xgb.cv(data =  dtrain, nrounds = ntrees, nfold = 2, showsd =
                               TRUE, 
                            metrics = "rmse", verbose = TRUE, "eval_metric" = "rmse",
                            "objective" = "reg:linear", "max.depth" = currentDepth,
                            "eta" = currentEta,                               
    ...
    output <- return(c(rmse, trmse, currentSubsampleRate, currentColsampleRate,
                       currentDepth, currentEta,
  ...
  bst <- xgb.train(data=dtrain, eta=0.7, gamma = 1, max_depth = 8,
                ...
                nrounds=40, watchlist=watchlist, nthread = 4, objective =
                  "reg:linear")
...
### 3.0 Baseline: Average-Feature Prediction

First of all, we just use the mean of our test features obtained from 1st Stack models as the predictions of sales on test set, call this Baseline method, and see the performance.
```{r}
...
print(paste0("the score for Baseline method is ", avg_kaggle_score))
```
We could see that the performance of the Baseline (Average-Feature Prediction) method is not very good.
...
# need to read test set again as the dataframe changes
test <- fread("../output/combined_test.csv")
```
We can see from the results that Linear Regression performs even worse than the Baseline(Average-Feature Prediction) method.
...
```
We can see that Random Forest performs better than Baseline (Average-Feature Prediction) as it reduces the error score by over 8% (= 1-score_rf/avg_kaggle_score).
...
run.gbm = FALSE 
if(run.gbm){
  comb_test_gbm<-comb_test[,-c(1,2,3,4,5,7)]
  comb_valid_gbm<-comb_valid[,-c(1,2,3,4,5,7)]

  n.trees = c(300,500,800,1000)
  ...
  # sample cv
  nfolds=5
  cv_indices<-sample(nfolds, nrow(comb_valid_gbm),replace=TRUE)
  niter = length(n.trees)*length(interaction.depth)*length(shrinkage)*nfolds
  cv_res = data.frame(ntree = rep(NA,niter),interactiondepth = rep(NA,niter),
    ...
    for (j in 1:length(interaction.depth)){
      for (k in 1:length(shrinkage)){
        for (l in 1:nfolds){
          ...
          cv_res$ntree[counter] <- n.trees[i]
          cv_res$interactiondepth[counter] <- interaction.depth[j]
          cv_res$shrinkage[counter] <- shrinkage[k]
        
          test.cv <- comb_valid_gbm[cv_indices == l,]
          test.cv$unit_sales[test.cv$unit_sales < 0] <- 0
          train.cv <- comb_valid_gbm[cv_indices != l,]
          train.cv$unit_sales[train.cv$unit_sales < 0] <- 0
        
          model <- gbm(train.cv$unit_sales~.,data = train.cv,distribution="gaussian",
                      n.trees = n.trees[i],interaction.depth = interaction.depth[j],
                      shrinkage = shrinkage[k])
          pred <- predict(model, newdata=test.cv,n.trees=n.trees[i])
          pred[pred < 0] <- 0
          w <- ifelse(test.cv$perishable == 0, 1, 1.25)
          cv_res$score.gbm[counter] <- sqrt(sum(w * (log(pred + 1) -
                                                       log(test.cv$unit_sales +
                                                             1))^2)/sum(w))
          counter = counter + 1
        }
      }
  ...
  save(cv_res,file="../output/GBM_cv_res.RData")
}else{
  best_parameter <- c(400,2,0.15)
}
# find best parameter
#best_parameter<-cv_res[which.min(cv_res$score.gbm),]

print(paste0("The best n.trees is ",best_parameter[1],", the best interation.depth is ",best_parameter[2],", the best shrinkage is ",best_parameter[3], ", and the distribution is Gaussian."))

if(run.gbm){
  best_model<-gbm(train.cv$unit_sales~.,data = train.cv,distribution="gaussian",
                  n.trees = best_parameter[1],interaction.depth = best_parameter[2],
                  shrinkage = best_parameter[3])
  pred<-predict(best_model, newdata=comb_test_gbm,n.trees=best_parameter[1])
  pred[pred < 0] <- 0
  save(pred, file="../output/gbm_pred.RData")
}else{
  load("../output/gbm_pred.RData")
}
pred_gbm <- 
w <- ifelse(comb_test_gbm$perishable == 0, 1, 1.25)
comb_test$unit_sales[comb_test$unit_sales < 0] <- 0
best_score <- sqrt(sum(w * (log(pred_gbm + 1) - log(comb_test$unit_sales + 1))^2)/sum(w))

print(paste0("the score for GBM in stack 2 is ", best_score))
```
We can see that GBM performs better than Baseline (Average-Feature Prediction).

### 3.4 XGBoost

```{r}
run_xgb_stack2 <- FALSE

if(run_xgb_stack2){
  features <- c('rf_pred', 'xgboost_pred', 'ets_pred', 'prophet_pred', 'arima_pred')

  comb_valid$unit_sales[comb_valid$unit_sales< 0] <- 0
  comb_test$unit_sales[comb_test$unit_sales< 0] <- 0

  dtrain <- xgb.DMatrix(data = as.matrix(comb_valid[, features]), label =
                          comb_valid$unit_sales)
  dtest <- xgb.DMatrix(data = as.matrix(comb_test[, features, with=FALSE]), label =
                         comb_test$unit_sales)
  ...
  if(tuning){
    ...
    xgboostModelCV <- xgb.cv(data =  dtrain, nrounds = ntrees, nfold = 2, showsd =
                               TRUE, 
                             metrics = "rmse", verbose = TRUE, "eval_metric" =
                               "rmse",
                             "objective" = "reg:linear", "max.depth" = currentDepth,
                             "eta" = currentEta,                               
                             "subsample" = currentSubsampleRate, "colsample_bytree" =
                               currentColsampleRate
                             , print_every_n = 10, "min_child_weight" =
                               currentMinChild, "gamma" = currentGamma,
                             booster = "gbtree",
  ...
  output <- as.data.frame(t(rmseErrorsHyperparameters))
  varnames <- c("TestRMSE", "TrainRMSE", "SubSampRate", "ColSampRate", "Depth",
                "eta", "currentMinChild", "Gammma")
  names(output) <- varnames

  save(output, file = "../output/xgb2_cverror.RData")
  }

  bst <- xgb.train(data=dtrain, eta=0.01, gamma = 1, max_depth = 8,
                   min_child_weight = 5, subsample = 0.6, colsample_bytree = 0.6,
                   nrounds=200, watchlist=watchlist, nthread = 4, objective =
                     "reg:linear")

  preds2 <- predict(bst, dtest)
  # in case it predicts below 0, but it shouldn't.
  preds2[preds2 < 0] <- 0

  save(preds2, file="../output/test_pred_XGB2.RData")
}else{
  load("../output/test_pred_XGB2.RData")
}
w <- ifelse(comb_test$perishable == 0, 1, 1.25)
comb_test$unit_sales[comb_test$unit_sales < 0] <- 0
xgb_score <- sqrt(sum(w * (log(as.numeric(preds2) + 1) - log(as.numeric(comb_test$unit_sales) + 1))^2)/sum(w))

print(paste0("the score for Xgboost in stack 2 is ", xgb_score))
```
We can see that XgBoost performs better than Baseline (Average-Feature Prediction) as it reduces the error score by over 9% (= 1-xgb_score/avg_kaggle_score).
## Step 4: Conclusion
As Random Forest, and XgBoost perform well on 2nd Stack, we would like to try the combinations of these two models.
```{r}
# Combine the predictions from the two models as a matrix
comb_mat <- cbind(pred_rf, preds2)

# Use the average of the predictions from these three models as the final prediction
final.pred <- rowMeans(comb_mat)
final.score <- sqrt(sum(w * (log(final.pred + 1) - log(as.numeric(comb_test$unit_sales) + 1))^2)/sum(w))

print(paste0("the final score is ", final.score))
```
As the combinations of the predictions from 2nd Stack models Random Forest, and XgBoost do give us better performance, we would like to include this step as our model.