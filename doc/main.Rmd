...
Here we use ridge regression for the second stack, as it allows for some variable/feature selection/importance via shrinkage, while not necessarily shrinking the coefficients to 0 (since we believe that all of the models will have some predictive power still). We also use a linear regression that takes an average of the other model outputs.
...
avg_kaggle_score = sqrt(sum(w * (log(test$mean_pred + 1) - log(test$unit_sales + 1))^2)/sum(w))
...
if(run_rf_stack2){
  ...
  # run the RF on random sampled data
  for (j in 1:length(n_tree2)){
    for(i in 1:K){
      train.data <- train_rf[s != i,]
      test.data <- train_rf[s == i,]
      ...
      # Get the 'mtry' for trained model
      opt.mtry2[i] <- fit$mtry
      pred <- predict(fit, test.data[,-c("unit_sales")])
      w <- ifelse(test.data$perishable == 0, 1, 1.25)
      score.rf2[i] <- sqrt(sum(w * (log(pred + 1) - log(test.data$unit_sales + 1))^2)/sum(w))
    }
    # Get the lowest error rate of cross validation
    score.tree[j] <- mean(score.rf2)
    mtry.tree[j] <- opt.mtry2[which.min(score.rf2)]
  }
  save(score.tree, file="../output/RF2/score_tree.RData")
  save(mtry.tree, file="../output/RF2/mtry_tree.RData")
}else{
  load("../output/RF2/score_tree.RData")
  load("../output/RF2/mtry_tree.RData")
}
best_tree <- n_tree2[which.min(score.tree)]
best_mtry <- mtry.tree[which.min(score.tree)]
names(score.tree) <- as.character(n_tree2)
score.tree
print(paste0("Best ntree parameter is ",best_tree,", and best mtry parameter is ",best_mtry,"."))

if(run_rf_stack2){
  test_rf <- comb_test[,c("date","store_item_nbr.x","unit_sales","rf_pred","xgboost_pred","ets_pred","prophet_pred","arima_pred","perishable")]
  fit <- tuneRF(train.data[,-"unit_sales"], train.data$unit_sales,
                    ntreeTry = n_tree2[j], 
                    doBest = TRUE)
