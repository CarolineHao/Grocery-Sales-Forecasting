...
dtrain <- xgb.DMatrix(data = as.matrix(comb_valid[, features]), label = comb_valid$unit_sales)
dtest <- xgb.DMatrix(data = as.matrix(comb_test[, features, with=FALSE]), label = comb_test$unit_sales)
...
bst <- xgb.train(data=dtrain, eta=0.01, gamma = 1, max_depth = 8,
                 ...
                 nrounds=200, watchlist=watchlist, nthread = 4, objective = "reg:linear")
...
print(paste0("the score for RF in stack 2 is ", valid_score))
}
```