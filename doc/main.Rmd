...
```{r}
library(gbm)
load("/Users/lijingkai/Documents/fall2017-project5-group1/output/comb_validFeatures.RData")
load("/Users/lijingkai/Documents/fall2017-project5-group1/output/comb_testFeatures.RData")
  ...
  for (j in 1:length(interaction.depth)){
    for (k in 1:length(shrinkage)){
      for (l in 1:nfolds){
        ...
        cv_res$score.gbm[counter] <- sqrt(sum(w * (log(pred + 1) - log(test.cv$unit_sales + 1))^2)/sum(w))
