...
fcst_matrix <- foreach(i=1:nrow(train_ts),.combine=rbind, .packages=c("forecast")) %dopar% { 
  fcst_matrix <- forecast(ets(train_ts[i,], lambda = 0.5),h=fcst_intv)$mean
}
...
save(train_save, file = paste0("./output/ets_lambda_0.5.RData"))
# calculate the score of accuracy prediction on validation set
w <- ifelse(train_comb$perishable == 0, 1, 1.25)
score1 <- sqrt(sum(w * (log(train_comb$sales_pred + 1) - log(train_comb$unit_sales + 1))^2)/sum(w))
...
fcst_matrix1 <- foreach(i=1:nrow(train_ts1),.combine=rbind, .packages=c("forecast")) %dopar% { 
  fcst_matrix1 <- forecast(ets(train_ts1[i,], lambda = 0.5),h=fcst_intv1)$mean
}
