...
test_set$pre1 = predict(bst1,newdata=test_set,n.trees =320)
test_set$pre1[test_set$pre1 < 0] = 0
test_set$unit_sales[test_set$unit_sales < 0] = 0
test_set = as.data.frame(test_set)
save(test_set, file = "GBM_test_comb_1.RData")


w = ifelse(test_set$perishable == 0, 1, 1.25)
valid_score = sqrt(sum(w * (log(as.numeric(test_set$pre1) + 1) - log(as.numeric(test_set$unit_sales) + 1))^2)/sum(w))
valid_score
...
bst3 = gbm(unit_sales~rf_pred+xgboost_pred+ets_pred+prophet_pred+arima_pred, interaction.depth =
            ,data=train_set,distribution="gaussian",n.trees =320,verbose
          =F,shrinkage =0.15)
